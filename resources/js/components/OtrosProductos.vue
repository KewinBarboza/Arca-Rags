        ...
        <div class="row">
            ...
            <div class="col-sm-12 col-md-3 col-lg-3 float-left" v-for="(item , index) of productos.data" :key="index">
                <div class="card card-sombra border-0" v-if="index <= 3" >
                    <div v-for="(img , index) of item.imagen"  :key="index">
                        <img :src="img.url" v-if="index===0" class="mx-auto d-block card-img-top p-3 "  :alt="item.nombre">  
                    </div>

                    <div class="card-body p-2">
                        <h5 class="card-title mb-1">{{ item.nombre }}.</h5>
                        <p class="card-text color-parrafo mb-1">{{ item.descripcion }}.</p>
                        <router-link class="text-decoration-none"  :to="{name:'Producto', params:{url: item.id}}" ><b @click="recargar()">Ver mas</b></router-link>
                    </div>
                </div>
            </div>
        </div>
